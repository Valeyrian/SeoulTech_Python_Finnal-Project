...
class MovieCard(QFrame):
    ...
    Signals:
        like_changed (str, bool): Emitted when the like status changes (movie_id, is_liked)
        play_clicked (object): Emitted when the play button is clicked (movie object)
    """
    ...
    def create_image_container(self, parent_layout):
        ...
        Args:
            parent_layout: Parent layout to add the container to
        """
    ...
    def create_text_section(self, parent_layout):
        ...
        Args:
            parent_layout: Parent layout to add the text section to
        """
    ...
    def update_like_button_state(self):
        """Update the like button appearance based on current state."""
...
def create_movie_card(movie, user_manager=None):
    """
    Factory function for creating a movie card.
    Creates and returns a MovieCard instance.
    ...
    Returns:
        MovieCard: Movie card instance
    """
    return MovieCard(movie, user_manager)


def delete_movie_card(card):
    """
    Helper function to delete a movie card.
    
    Args:
        card: MovieCard instance to delete
    """
    card.setParent(None)
    card.deleteLater()
...
FilmCard = MovieCard
createFilmCard = create_movie_card
deleteFilmCard = delete_movie_card
